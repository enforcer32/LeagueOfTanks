...
#include "LOTEngine/Renderer/Framebuffer.h"
...
namespace LOT
{
	...
	{
		LOT_ASSERT(properties.Attachments.size() <= 6, "Framebuffer only supports 6 Color Attachments")
		Reset();
	}
	...
	void Framebuffer::Reset()
	{
		...
		LOT_ASSERT(glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE, "glCheckFrameBufferStatus(FrameBuffer FAILED)");
		glBindFramebuffer(GL_FRAMEBUFFER, 0);
	}
}
