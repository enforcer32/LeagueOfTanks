...
namespace LOT
{
	...
	{
		LOT_LOG_ERROR("[OpenGL Error]({}) {}", type, message);
	}
	...
	bool Window::InitContext()
	{
		...
		if (!m_NativeWindow)
		{
			LOT_LOG_ERROR("FAILED glfwCreateWindow for {}", m_WinPrivData.Name);
			glfwTerminate();
			return false;
		}
		...
		{
			LOT_LOG_ERROR("FAILED gladLoadGLLoader");
			glfwTerminate();
			return false;
		}
		...
		//LOT_LOG_INFO("OpenGL: Version({}), Renderer({}), Vendor({}) ", glGetString(GL_VERSION), glGetString(GL_RENDERER), glGetString(GL_VENDOR));
