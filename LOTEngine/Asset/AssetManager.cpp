...
namespace LOT
{
	...
	{
		LOT_LOG_DEBUG("AssetManager Initializing...");
		return true;
	}

	void AssetManager::OnShutdown()
	{
		LOT_LOG_DEBUG("AssetManager Shutting Down...");
	}
	...
	bool AssetManager::HasAssetID(AssetID assetID)
	{
		if (s_Assets.find(assetID) != s_Assets.end())
			return true;
		LOT_LOG_DEBUG("AssetManager AssetID Not Found");
		return false;
	}
	...
	void AssetManager::ReloadAsset(AssetID assetID)
	{
		if (HasAssetID(assetID))
			if (s_Assets[assetID]->Reload())
				LOT_LOG_DEBUG("Reloaded Asset({})", (uint64_t)assetID);
			else
				LOT_LOG_DEBUG("AssetManager Failed To Reload " + (uint64_t)assetID);
	}
